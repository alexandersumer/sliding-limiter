...
Suppose each bucket represents 1 second. When a requst comes in we get the current UNIX time and round it to the nearest integer value (time in seconds since EPOCH). This will be the key for the bucket in this requestors bucket->count hashmap. if the bucket contains a value, then we incrment it, elsif the bucket contains no value, then we set the value to 1.
